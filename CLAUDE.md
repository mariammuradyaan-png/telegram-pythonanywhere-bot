...
| `IMAGINE_WIDTH` / `IMAGINE_HEIGHT` | No | `1024` | Output image dimensions |
...
| `BROADCAST_SECRET` | No | — | Enables `/api/broadcast` (daily morning motivational messages). Fail-closed: unset ⇒ endpoint returns 403. Set the same value as a GitHub repo secret `BROADCAST_SECRET` so `.github/workflows/motivation.yml` can call it. Requires `SQLITE_PATH` (the subscriber list lives in the store) |
| `MOTIVATION_PROMPT` | No | _morning preset_ | Instruction handed to the AI (with `SYSTEM_PROMPT`) to compose the daily message. Override to change its flavour |
| `HOROSCOPE_PROMPT` | No | _reading preset_ | Instruction for the daily horoscope; the literal `{sign}` is replaced with the subscriber's sign. `/api/horoscope` reuses `BROADCAST_SECRET` (no separate secret) |
...
- **Daily motivational broadcast has no server-side scheduler** — PA free tier has no cron/scheduled tasks, so daily morning messages are driven the same way as deploys: a GitHub Actions cron (`.github/workflows/motivation.yml`, `0 4 * * *` UTC) POSTs the secret-protected `/api/broadcast` endpoint each morning. `/subscribe` / `/unsubscribe` store the opt-in list as a single JSON blob under `motivation:subscribers` in the store (`bot/motivation.py`) — one key because the KV store has no scan/list op. `broadcast_motivation()` generates **one** message (single AI call) and sends it to everyone; per-user send failures are logged and skipped, and users who blocked the bot (Telegram `error_code == 403`) are auto-unsubscribed. Requires `SQLITE_PATH` (no store ⇒ `/subscribe` tells the user it's unavailable). The GitHub Actions cron is UTC and best-effort (GitHub may delay runs a few minutes) — fine for a good-morning. `/api/broadcast` only 500s when generation fails, which is *before* any send, so the cron's `--retry` can't double-send
- **Daily horoscope captures the sign once, sends per-sign** — `/horoscope <sign|birthday>` registers a user and stores their zodiac sign under `horoscope:subscribers` (`bot/horoscope.py`); registration parses either a sign name or a birthday (`parse_sign` → `sign_from_date`, month-first dates). It never re-asks: `/horoscope` with no arg reads today's stars for an already-registered user, or prompts once for a new one; `/horoscope stop` opts out. `broadcast_horoscopes()` groups subscribers by sign and generates **one** reading per distinct sign (≤12 AI calls, not one per user), then sends each person theirs. Driven by `/api/horoscope` (reuses `BROADCAST_SECRET`) + the `.github/workflows/horoscope.yml` cron (`0 5 * * *` UTC, staggered an hour after the motivation broadcast). Needs a `PA_HOROSCOPE_URL` GitHub secret. Same 403-auto-unsubscribe and generate-before-send (retry-safe) properties as the motivation broadcast
- **`/imagine` sidesteps the PA outbound whitelist** — `bot/handlers.py::cmd_imagine` builds a Pollinations image URL (`bot/imagine.py::build_image_url`) and passes it to `bot.send_photo(chat_id, url)`. Telegram's servers fetch that URL, so the PA worker makes **no** outbound call to the image host — the free-tier whitelist never applies and no image API key is needed. If Telegram fails to fetch/generate (slow model, transient error) the handler falls back to sending the raw link. The romantic look comes from `IMAGINE_STYLE` being appended to every prompt, and a per-message `seed` (the Telegram `message_id`) keeps repeated prompts from returning the same cached image
